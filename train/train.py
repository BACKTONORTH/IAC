"""
The code is based on https://github.com/huggingface/diffusers/blob/main/examples/dreambooth/train_dreambooth.py
"""
import os
...
class DataDreamDataset(Dataset):
    def __init__(
        self,
        fewshot_data_dir,
        tokenizer,
        ...
        if caption_path and os.path.exists(caption_path):
            ...
            # 如果未提供caption_path或文件不存在，使用空字典
            logger.warning(f"Caption file not found, using class names as captions")
            self.captions = {}
        ...
        if not self.fewshot_data_dir.exists():
            raise ValueError(f"Instance images root doesn't exist: {self.fewshot_data_dir}")
    ...
    def get_instance_data_list(self):
        ...
        instance_prompts = []
        for cls_idx, classname in enumerate(SUBSET_NAMES[self.dataset]):
            ...
            # Get image filename and corresponding caption
            for img_path in images_path:
                img_filename = os.path.basename(img_path)
                caption = self.captions.get(img_filename, classname)  # 如果找不到caption就用类名
                instance_prompts.append(caption)
                instance_images_path.append(img_path)
    ...
    def __getitem__(self, index):
        ...
        # 添加数据集前缀,比如"satellite"对于eurosat数据集
        instance_prompt = f"a {self.dataset_name}{instance_prompt}"
